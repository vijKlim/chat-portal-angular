#!/usr/bin/env php
<?php
//Yii::import('application.components.WSocketDaemonHandler');
require_once(__DIR__.'/../../components/WSocketDaemonHandler.php');
class Server
{
    protected $config;
...
// Pass Phrase (password) of private key
        ...
        if (!empty($this->config['ssl'])) {
            ...
            //for wss
            $server = stream_socket_server($this->config['ssl'], $errorNumber, $errorString,STREAM_SERVER_BIND|STREAM_SERVER_LISTEN, $context);

            //когда использовал ws то эта строка была разкомментированна, но для wss нужно закомментировать
...
//я не использую сокет для внутриних сообщений на сервере
...
//можно задавать алиасы чтоб подключать указанный в конфиге eventDriver(класс Daemon расширяет класс Generic (то есть псевдоним), но я на прямую расширил его класом GenericSelect)
//        if (!empty($this->config['eventDriver']) && $this->config['eventDriver'] == 'libevent') {
//            class_alias('GenericLibevent', 'Generic');
//        } elseif (!empty($this->config['eventDriver']) && $this->config['eventDriver'] == 'event') {
//            class_alias('GenericEvent', 'Generic');
//        } else {
//            class_alias('GenericSelect', 'Generic');
//        }

        file_put_contents($this->config['pid'], getmypid()/*posix_getpid()*/);
        ...
        $worker = new WSocketDeamonHandler($server, $service, $master);//new $workerClass ($server, $service, $master);
    ...
    public function stop() {
        $pid = @file_get_contents($this->config['pid']);
        if ($pid) {
            shell_exec('kill '.$pid.' '.SIGTERM);//posix_kill($pid, SIGTERM);
